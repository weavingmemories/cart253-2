...
String tonePattern = "1-2-3-2-3-4-3-2-";
...
void setup() {
  ...
  for (int i = 0; i < frequencies.length; i++) {
    ...
    for (int k = 0; k < drops.length; k++) {
      ...
      drops[k] = new Rain(); // Create each rain particle object
      r1 = new Rain();
    }
  }
}
...
void draw() {
  ...
  // If the brightness of the screen is white-light grey, change the toneBeat to:
  if (averageBrightness >=0 && averageBrightness <= 85) {
    tone1.amp(0.5);
    tone2.amp(0.5);
    tone3.amp(0.5);
    tone4.amp(0.5);
  ...
  // If the brightness of the screen is medium grey, change the toneBeat to:
  else if (averageBrightness >= 86 || averageBrightness <= 170) {
    tone1.amp(1);
    tone2.amp(1);
    tone3.amp(1);
    tone4.amp(1);
  ...
  // If the brightness of the screen is a dark grey-black, change the toneBeat to:
  else {
    tone1.amp(10);
    tone2.amp(10);
    tone3.amp(10);
